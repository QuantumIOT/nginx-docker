...
# catch current IPs
current_ips=$(curl --unix-socket /var/run/docker.sock http:/v1.30/tasks | jq -c '.[] | {addresses: .NetworksAttachments[].Addresses[], id: .ID}');
...
while /bin/true; do
  ...
  new_ips=$(curl --unix-socket /var/run/docker.sock http:/v1.30/tasks | jq -c '.[] | {addresses: .NetworksAttachments[].Addresses[], id: .ID}')
