...
curl -s --unix-socket /var/run/docker.sock http:/v1.30/tasks >/dev/null 2>&1
...
# catch current IPs
current_ips=$(curl -s --unix-socket /var/run/docker.sock http:/v1.30/tasks | jq -r -j ".[] | .NetworksAttachments[].Addresses[] + .ID");
...
while /bin/true; do
  ...
  new_ips=$(curl -s --unix-socket /var/run/docker.sock http:/v1.30/tasks | jq -r -j ".[] | .NetworksAttachments[].Addresses[] + .ID");
